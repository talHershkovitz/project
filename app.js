...
app.get("/user", function (req, res) {
    sql.query("SELECT * FROM useres", (err, mySQLrespons) => {
        if (err) {
            ...
            res.status(400).send({
                message: "error in getting all records: " + err
            });
            return;
        }
...
app.get("/qa", function (req, res) {
    ...
    sql.query("SELECT * FROM subject", (err, mySQLrespons) => {
        if (err) {
            console.log("error: ", err);
            res.status(400).send({
                message: "error in getting all records of subject: " + err
            });
            return;
        }
        ...
        mySQLrespons.forEach(function (row) {
            var sub_num = row.id;
            var sub = row.subject;
            sql.query(`select * from qa where subject = ${sub_num}`, (err, ans) => {
                if (err) {
                    console.log("error: ", err);
                    res.status(400).send({
                        message: "error in getting all records of qa: " + err
                    });
                    return;
                }
...
app.get("/review", function (req, res) {
    sql.query(`SELECT * FROM positive_db.reviews `, (err, mySQLrespons) => {
        if (err) {
            console.log("db error: ", err);
            res.status(400).send({
                message: "error in getting all records: " + err
